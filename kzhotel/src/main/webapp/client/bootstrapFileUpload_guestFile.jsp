...
<body>
    ...
    <script src="${pageContext.request.contextPath }/js/bsupload/zh.js" type="text/javascript"></script>
		...
		<input id="file-editGuestFile" name="file" class="file" type="file" multiple data-min-file-count="1"/>
		<script>
	$("#file-editGuestFile").fileinput({
		...
		,uploadUrl :'../upfile/uploadGuestPhoto'
	});
	$('#file-editGuestFile').on('fileuploaded',function(event, data, previewId, index) {
		var form = data.form, 
		files = data.files,
		extra = data.extra,
		response = data.response,
		reader = data.reader;
		parent.document.getElementById("picture_guestFile").setAttribute("src",parent.eapor.data.bootStrapFileUploadUrl+"/upload/uploadGuest/"+ response.statusMsg);
		//parent.closeUpFileDiv_editroomtype();//关闭图片上传组件
		parent.$('#upFileClose_guestFile').trigger('click');
